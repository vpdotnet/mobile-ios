...
final class SignupEmailIntegrationTests: XCTestCase {
    ...
    var capturedUserAccount: VP_VPN_tvOS.UserAccount?
    
    func instantiateSut(accountProviderResult: (VPLibrary.UserAccount?, Error?), onSuccessAction: @escaping (VP_VPN_tvOS.UserAccount) -> Void) {
    ...
    func test_signup_succeeds_when_a_valid_userAccount_is_retrieved() throws {
        ...
        let userAccountStub = VPLibrary.UserAccount.makeStub()
        let error: Error? = nil
    ...
    func test_signup_shows_an_invalid_email_error_when_the_email_is_not_valid() throws {
        ...
        instantiateSut(accountProviderResult: (VPLibrary.UserAccount.makeStub(), error)) { [weak self] userAccount in
            self?.capturedUserAccount = userAccount
            expectation.fulfill()
        }
    ...
    func test_signup_shows_a_generic_error_when_there_is_no_error_and_no_userAccount() throws {
        ...
        let userAccountStub: VPLibrary.UserAccount? = nil
        let error: Error? = nil
        let expectation = expectation(description: "Waiting for signup to update")
