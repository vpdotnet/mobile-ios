...
final class SignupIntegrationTests: XCTestCase {
    ...
    func instantiateSut(resultGetAvailableProductsUseCase: ([Plan : InAppProduct]?, Error?), resultPurchaseProductUseCase: (InAppTransaction?, Error?), appStoreInformationResult: VPLibrary.AppStoreInformation?, onSuccessAction: @escaping (InAppTransaction?) -> Void) {
        let accountProviderMock = AccountProviderMock(userResult: VPLibrary.UserAccount.makeStub(),
                                                      errorResult: nil,
    ...
    func test_getproducts_successfully_presents_products_when_it_succeeded_retrieving_products() throws {
        ...
        let appStoreInformation: VPLibrary.AppStoreInformation? = nil
