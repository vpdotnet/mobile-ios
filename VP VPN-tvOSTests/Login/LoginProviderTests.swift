...
final class LoginProviderTests: XCTestCase {
    ...
    var capturedResult: Result<VP_VPN_tvOS.UserAccount, Error>?
    
    func instantiateSut(accountProviderResult: (VPLibrary.UserAccount?, Error?)) {
    ...
    func test_login_succeeds_when_accountprovider_completes_with_user_and_no_error() throws {
        // GIVEN
        let user = VPLibrary.UserAccount.makeStub()
        let error: Error? = nil
        
        instantiateSut(accountProviderResult: (user, error))
        
        let credentials = VP_VPN_tvOS.Credentials(username: "", password: "")
        ...
        switch (capturedPlan, userPlan) {
            case (Plan.monthly, Plan.monthly), (Plan.yearly, Plan.yearly), (Plan.trial, VPLibrary.Plan.trial), (Plan.other, Plan.other):
                XCTAssertTrue(true)
            default:
    ...
    func test_login_fails_when_accountprovider_completes_with_expired_user() throws {
        // GIVEN
        let user = VPLibrary.UserAccount.makeExpiredStub()
    ...
    func test_login_fails_when_accountprovider_completes_with_no_user_and_error() throws {
        // GIVEN
        let user: VPLibrary.UserAccount? = nil
