...
class AppPreferences {
...
#endif
    var lastVPNConnectionStatus: VPLibrary.VPNStatus {
        get {
    ...
    var favoriteServerIdentifiersGen4: [String] {
        get {
            let keychain = VPLibrary.Keychain(team: AppConstants.teamId, group: AppConstants.appGroup)
    ...
    func migrateOVPN() {
        ...
        guard let currentOpenVPNConfiguration = Client.preferences.vpnCustomConfiguration(for: VPTunnelProfile.vpnType) as? OpenVPNProvider.Configuration ??
    ...
    func migrate() {
        ...
        let oldKeychain = VPLibrary.Keychain()
        let newKeychain = VPLibrary.Keychain(team: AppConstants.teamId, group: AppConstants.appGroup)
...
//        NSString *subscriptionPlan = [oldDefaults objectForKey:@"SubscriptionPlan"];
    ...
    private func migrateAPItoV2() {
        ...
        if let oldUsername = defaults.string(forKey: "LoggedUsername"),
            let _ = try? VPLibrary.Keychain(team: AppConstants.teamId,
                                             group: AppConstants.appGroup).password(for: oldUsername) {
            //User is loggedIn
            try? VPLibrary.Keychain().set(username: oldUsername)
            try? VPLibrary.Keychain().set(publicUsername: oldUsername)
