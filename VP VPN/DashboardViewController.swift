...
class DashboardViewController: AutolayoutViewController {
    ...
    @IBOutlet private weak var toggleConnection: VPConnectionButton!
    ...
    override func viewDidLoad() {
        ...
        nc.addObserver(self, selector: #selector(accountDidLogout(notification:)), name: .VPAccountDidLogout, object: nil)
        ...
        nc.addObserver(self, selector: #selector(closeSession), name: .VPAccountLapsed, object: nil)
        nc.addObserver(self, selector: #selector(reloadTheme), name: .PIAThemeShouldChange, object: nil)
        nc.addObserver(self, selector: #selector(checkAccountEmail), name: .VPAccountDidRefresh, object: nil)
    ...
    private func handleNonCompliantWifiConnection() {
        ...
        let isOpenVPNSelected = selectedProtocol == VPTunnelProfile.vpnType.vpnProtocol
...
extension DashboardViewController {
    @available(iOS 16.2, *)
    private func makeLiveActivityStateForCurrentConnection() -> VPConnectionAttributes.ContentState {
        ...
        let state = VPConnectionAttributes.ContentState(connected: vpnProvider.isVPNConnected, regionName: currentServer.name, regionFlag: "flag-\(currentServer.country.lowercased())", vpnProtocol: vpnProtocol)
        return state
    }
