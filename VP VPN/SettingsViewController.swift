...
class SettingsViewController: AutolayoutViewController, SettingsDelegate {
    ...
    var pendingWireguardVPNConfiguration: VPWireguardConfiguration!
    ...
    func updateDataEncryption(encryption value: String) {
        if pendingPreferences.vpnType == VPTunnelProfile.vpnType {
            ...
            case .dns:
                if let settingValue = value as? String {
                    if pendingPreferences.vpnType == VPTunnelProfile.vpnType {
                        ...
                        pendingWireguardVPNConfiguration = VPWireguardConfiguration(customDNSServers: DNSList.shared.valueForKey(settingValue), packetSize: AppPreferences.shared.wireGuardUseSmallPackets ? AppConstants.WireGuardPacketSize.defaultPacketSize : AppConstants.WireGuardPacketSize.highPacketSize)
                    }
                }
            }
        }
    ...
    @objc private func refreshWireGuardSettings() {
        guard let currentWireguardVPNConfiguration = Client.preferences.vpnCustomConfiguration(for: PIAWGTunnelProfile.vpnType) as? VPWireguardConfiguration ??
            Client.preferences.defaults.vpnCustomConfiguration(for: PIAWGTunnelProfile.vpnType) as? VPWireguardConfiguration else {
    ...
    private func doReset() {
        ...
        guard let currentOpenVPNConfiguration = pendingPreferences.vpnCustomConfiguration(for: VPTunnelProfile.vpnType) as? OpenVPNProvider.Configuration else {
        ...
        pendingWireguardVPNConfiguration = VPWireguardConfiguration(customDNSServers: [], packetSize: AppConstants.WireGuardPacketSize.defaultPacketSize)
    ...
    private func commitAppPreferences() {
        ...
        if #available(iOS 14.0, *) {
            WidgetCenter.shared.reloadTimelines(ofKind: "VPWidget")
        }
    ...
    @objc func reloadSettings() {
        ...
        guard let currentOpenVPNConfiguration = pendingPreferences.vpnCustomConfiguration(for: VPTunnelProfile.vpnType) as? OpenVPNProvider.Configuration ??
            Client.preferences.defaults.vpnCustomConfiguration(for: VPTunnelProfile.vpnType) as? OpenVPNProvider.Configuration else {
            fatalError("No default VPN custom configuration provided for PIA OpenVPN protocol")
        }
        
        guard let currentWireguardVPNConfiguration = pendingPreferences.vpnCustomConfiguration(for: PIAWGTunnelProfile.vpnType) as? VPWireguardConfiguration ??
            Client.preferences.defaults.vpnCustomConfiguration(for: PIAWGTunnelProfile.vpnType) as? VPWireguardConfiguration else {
            fatalError("No default VPN custom configuration provided for PIA Wireguard protocol")
        }
        ...
        if pendingPreferences.vpnType == VPTunnelProfile.vpnType {
            ...
            if pendingPreferences.vpnType == PIAWGTunnelProfile.vpnType {
                if AppPreferences.shared.wireGuardUseSmallPackets {
                    pendingPreferences.setVPNCustomConfiguration(VPWireguardConfiguration(customDNSServers: pendingWireguardVPNConfiguration.customDNSServers, packetSize: AppConstants.WireGuardPacketSize.defaultPacketSize), for: pendingPreferences.vpnType)
                } else {
                    pendingPreferences.setVPNCustomConfiguration(VPWireguardConfiguration(customDNSServers: pendingWireguardVPNConfiguration.customDNSServers, packetSize: AppConstants.WireGuardPacketSize.highPacketSize), for: pendingPreferences.vpnType)
                }
            }
        }
    ...
    private func validateDNSList() {
        if pendingPreferences.vpnType == PIAWGTunnelProfile.vpnType {
            if Flags.shared.enablesDNSSettings {
                       ...
                       if !isValid {
                        pendingWireguardVPNConfiguration = VPWireguardConfiguration(customDNSServers: [], packetSize: AppPreferences.shared.wireGuardUseSmallPackets ? AppConstants.WireGuardPacketSize.defaultPacketSize : AppConstants.WireGuardPacketSize.highPacketSize)
                       }
            }
        } else {
            if Flags.shared.enablesDNSSettings {
