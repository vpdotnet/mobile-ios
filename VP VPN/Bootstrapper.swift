...
class Bootstrapper {
    ...
    func bootstrap() {
        ...
        Client.configuration.addVPNProfile(VPTunnelProfile(bundleIdentifier: AppConstants.Extensions.tunnelBundleIdentifier))
        ...
        defaults.vpnCustomConfigurations = [
            VPTunnelProfile.vpnType: AppConfiguration.VPN.piaDefaultConfigurationBuilder.build(),
            PIAWGTunnelProfile.vpnType: VPWireguardConfiguration(customDNSServers: [], packetSize: AppConstants.WireGuardPacketSize.defaultPacketSize)
        ]
    #endif
