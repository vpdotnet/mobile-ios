...
class CustomDNSSettingsViewController: AutolayoutViewController {
    ...
    @objc private func update(_ sender: Any?) {
        if isValidForm() {
            ...
            DNSList.shared.addNewServerWithName((vpnType == VPTunnelProfile.vpnType ? DNSList.CUSTOM_OPENVPN_DNS_KEY : DNSList.CUSTOM_WIREGUARD_DNS_KEY),
                                                andIPs: ips)
            self.delegate?.updateSetting(NetworkSections.dns,
                                         withValue: (vpnType == VPTunnelProfile.vpnType ? DNSList.CUSTOM_OPENVPN_DNS_KEY : DNSList.CUSTOM_WIREGUARD_DNS_KEY))
    ...
    @objc private func clear(_ sender: Any?) {
        ...
        alertController.addActionWithTitle(L10n.Localizable.Global.ok) {
            if let firstKey = DNSList.shared.firstKey() {
                DNSList.shared.removeServer(name: (self.vpnType == VPTunnelProfile.vpnType ? DNSList.CUSTOM_OPENVPN_DNS_KEY : DNSList.CUSTOM_WIREGUARD_DNS_KEY))
                self.delegate?.updateSetting(NetworkSections.dns,
                                             withValue: firstKey)
            }
