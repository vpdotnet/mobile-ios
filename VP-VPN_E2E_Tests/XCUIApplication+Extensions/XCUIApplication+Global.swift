...
extension XCUIApplication {
    ...
    func loginAndInstallVPNProfile(from test: XCTestCase) {
        ...
        let usernameTextField = textField(with: VPLibraryAccessibility.Id.Login.username)
        
        let passwordTextField = secureTextField(with: VPLibraryAccessibility.Id.Login.password)
    ...
    func logOutIfNeeded() {
        ...
        let logOutButton = cell(with: VPLibraryAccessibility.Id.Menu.logout).firstMatch
        
        if logOutButton.waitForExistence(timeout: defaultTimeout) {
            logOutButton.tap()
            
            let confirmationDialogButton = button(with: VPLibraryAccessibility.Id.Dialog.destructive)
